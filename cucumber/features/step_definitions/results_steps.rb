...
Then(/^I should see (?:a list of|only) (\d+) results?$/) do |length|
  JSON.parse(@response.body)["results"].length.should == length.to_i
end
...
Then(/^I should see the most recent result$/) do
  JSON.parse(@response.body)["results"][0]["id"].should == @id
end

Then(/^I should a result for the url "(.*?)"$/) do |url|
  JSON.parse(@response.body)["results"][0]["data"]["u"].should == url
end

Then(/^there should be no results$/) do
  @response = Server.get('/results')
  JSON.parse(@response.body)["results"].length.should == 0
end